...
public class RemoveArgument extends PluginCommandArgument {

    @Override
    public void onCommand(CommandSender sender, String[] args) throws CommandException {
        if (args.length == 0) {
            throw new CommandException(Collections.singletonList("Invalid Usage: /opsync remove <player>"));
        }
        ...
        OPSync.getInstance().getDatabaseManager().isAuthorized(target.getUniqueId(), new GenericCallback() {
            ...
            public void call(boolean result, boolean error) {
                ...
                if (result) {
                    sender.sendMessage(ChatColor.GREEN + "You have removed that player's authorization.");
                    OPSync.getInstance().getDatabaseManager().removeAuthorized(target.getUniqueId());
                }
                else {
                    sender.sendMessage(ChatColor.RED + "That player is already not authorized.");
                }
            }
        });
    }
